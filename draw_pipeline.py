...
def draw_pipeline2(final_nodes, filename='graph', graph_format='png',
    text_font='Arial',monospace_font='Courier',suppress_arrowheads=False,
    final_view=True,
    show_legend=True,
    legend_order=['ts','tsdata','iv','metadata','tuningcurve','other'],
    ...
    if (suppress_arrowheads) :
        gmain.edge_attr.update(arrowhead='none')
    gmain.node_attr.update(shape='box',fontsize='8',fontname=monospace_font,
        margin='0.05,0.05',width='0.05',height='0.05')
    ...
    for n in all_nodes:
        ...
        if (n.vis_cluster):
            ...
            if not cxl: # if n.vis_cluster not in clusters:
                ...
                cx = cxl[0]
            cx.node(str(id(n)),label=label,shape=n.shape)
        else:
            g1.node(str(id(n)),label=label,shape=n.shape)
...
def draw_pipeline3(final_nodes, filename='graph', graph_format='png',
    text_font='Arial',monospace_font='Courier',suppress_arrowheads=False,
    final_view=True,
    show_legend=True,
